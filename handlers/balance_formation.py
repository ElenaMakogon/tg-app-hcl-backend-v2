...
class GoogleSheetsBalanceUpdater:
    ...
    async def update_balance(self, transaction: Dict[str, str]) -> str:
    ...
    async def _add_new_instance(self, transaction: dict, currency_col: str) -> str:
        ...
        # Устанавливаем значение для указанной валюты
        currency_index = currencies.index(transaction['Валюта'])
        #new_row[currency_index] = transaction['Сумма']
        """
                # Заполняем 0 для остальных валют
        for i in range(0, len(new_row)):
            if i != currency_index and currencies[i]:  # Пропускаем пустые заголовки
                new_row[i] = "0"
    ...
    async def _reFormatting(self, num: str): ## применяем при ЗАПИСИ строкового
        ...
        import re
        print(f"**********  работает _reFormatting, num = {num}")
        if type(num) == str and type(num) != None:
            ...
            if ("," and ".") in num_clear:
                ...
                print(f"**********  работает _reFormatting, num_clear = {num_clear}")
                num_clear = num_clear.replace(",", ".")
        else:
            return "0"
        return num_clear
